# Conhecimento da Hamsa (base do agente)

> Edite este arquivo com os fatos reais da corretora. Tudo aqui é lido pelo
> agente e usado para responder clientes com precisão. Quanto mais completo e
> correto, melhores as respostas. NÃO coloque senhas, tokens ou dados sensíveis
> de clientes. Depois de editar, reinicie o bot (Ctrl+C e rode de novo).
>
> As linhas começando com ">" e este bloco de instruções podem ficar — o agente
> entende. Preencha os campos entre colchetes [ ] e apague os exemplos que não
> se aplicam.

## Sobre a Hamsa
- Corretora: [nome completo / razão social]
- Atendimento humano: Concierge da Hamsa / Equipe Hamsa Group (o agente NUNCA
  cita nomes de pessoas ao cliente; sempre "o Concierge da Hamsa").
- Atuação: seguro-saúde internacional (IPMI) para pessoas e famílias com vida
  entre [Brasil e EUA / outros].
- Idiomas de atendimento: português, inglês, [espanhol/hebraico].
- Horário de atendimento humano: [ex.: seg–sex, 9h–18h (horário de Brasília)].

## Seguradoras com quem trabalhamos
> Liste só as reais. O agente pode citar estas ao cliente.
- [ex.: Cigna Global]
- [ex.: Allianz Care]
- [ex.: Bupa Global]
- [ex.: GeoBlue]
- [ex.: IMG]

## Produtos / planos que oferecemos
> Para cada um: para quem serve e o que cobre em linhas gerais.
- [Nome do plano] — [para quem] — [cobertura resumida]
- [Nome do plano] — [para quem] — [cobertura resumida]

## Áreas de cobertura
- [ex.: Mundial incluindo EUA]
- [ex.: Mundial exceto EUA]
- [ex.: Regional / Américas]

## Como funciona nossa cotação (o processo real)
> O agente descreve isso ao cliente e coleta os dados. Ajuste os passos.
1. Coletamos: nome, idade de cada pessoa, país/cidade de residência, países onde
   precisa de cobertura, se precisa de cobertura nos EUA, condições de saúde
   relevantes.
2. [ex.: O Concierge da Hamsa prepara comparativo com 2–3 seguradoras.]
3. [ex.: Retorno em até X dias úteis.]
4. [ex.: Contratação 100% online / assinatura digital.]

## Perguntas frequentes (com a resposta que queremos dar)
> Estas são as respostas "oficiais". O agente vai priorizar o que estiver aqui.
- **Vocês cobrem nos EUA?** [sua resposta]
- **Tem carência?** [sua resposta]
- **Cobre condição pré-existente?** [sua resposta]
- **Como funciona reembolso / sinistro?** [sua resposta em termos gerais]
- **Posso usar no Brasil e nos EUA?** [sua resposta]
- **Quanto custa?** Não informamos valores pelo agente; depende de idade, país e
  cobertura — o Concierge da Hamsa prepara a cotação. [ajuste se quiser]

## O que o agente NUNCA deve fazer
> Reforça as regras. Acrescente o que for específico do seu negócio.
- Não citar valores/preços específicos.
- Não prometer aprovação ou cobertura de condição específica.
- Não dar aconselhamento médico nem jurídico.
- [outras regras suas]

## Contatos e encaminhamento
- Quando escalar para o Concierge da Hamsa: [ex.: sinistro, emergência,
  cancelamento, cliente irritado, pedido explícito de humano].
- Em emergência médica: orientar o serviço de emergência local e o telefone 24h
  da seguradora no cartão da apólice.
